...
public class LDLTPermutedFactorization {
	...
	private static Log log = LogFactory.getLog(LDLTPermutedFactorization.class.getName());

	public LDLTPermutedFactorization(DoubleMatrix2D Q) throws JOptimizerException {
		this(Q, DIAGONAL_PIVOLTING_WITH_PARTIAL_PIVOTING, null);
	}
	
	public LDLTPermutedFactorization(DoubleMatrix2D Q, int mode) throws JOptimizerException {
		this(Q, mode, null);
	}
	
	public LDLTPermutedFactorization(DoubleMatrix2D Q, MatrixRescaler rescaler) throws JOptimizerException {
		this(Q, DIAGONAL_PIVOLTING_WITH_PARTIAL_PIVOTING, rescaler);
	}
	
	public LDLTPermutedFactorization(DoubleMatrix2D Q, int mode, MatrixRescaler rescaler) throws JOptimizerException {
		this.dim = Q.rows();
		this.Q = Q;
		this.mode = mode;
	 ...
	 */
	private  void pldltpt() throws JOptimizerException{
		...
		for(int j=0; j<n; j++){
			...
			if(Math.abs(ajj) > 1.e-16){
				...
				if(k<0){
					throw new JOptimizerException("singular matrix");
				}
	 ...
	 */
	private void pldltptBK() throws JOptimizerException {
		//matrix S will be changed by the factorization, ad we do not want to change the matrix passed in by the client
		DoubleMatrix2D S = (rescaler==null)? this.Q.copy() : this.Q;
		int n = S.rows();
		DoubleMatrix2D A = S.copy();
...
//					P.setQuick(v, p, 1);
			...
			try{
				//first mode
				//DPart = ALG.inverse(D.viewPart(j, j, 1 + s - 1, 1 + s - 1));
				//or (in alternative) second mode
				if(s==1){
					double det = ColtUtils.calculateDeterminant(D.viewPart(j, j, 1, 1));
					if(Math.abs(det) < Utils.getDoubleMachineEpsilon()){
						throw new JOptimizerException("Matrix is singular");
					}
					DPart = DoubleFactory2D.sparse.make(new double[][]{{1. / det}});
				}else if(s==2){
					DPart = ColtUtils.invert2x2Matrix(D.viewPart(j, j, 1 + s - 1, 1 + s - 1));
				}else{
