...
public class SOCPLogarithmicBarrier implements BarrierFunction {
	...
	public double value(DoubleMatrix1D X) {
		RealVector x = new ArrayRealVector(X.toArray());
	...
	public DoubleMatrix1D gradient(DoubleMatrix1D X) {
		...
		for (int i = 0; i < socpConstraintParametersList.size(); i++) {
			...
			ret = ColtUtils.sum(ret, DoubleFactory1D.dense.make(ret_i.toArray()));
		}
	...
	public DoubleMatrix2D hessian(DoubleMatrix1D X) {
		...
		for(int i=0; i<socpConstraintParametersList.size(); i++){
			...
			ret = ColtUtils.sum(ret, DoubleFactory2D.dense.make(ret_i.getData()));
		}
	...
	public double calculatePhase1InitialFeasiblePoint(DoubleMatrix1D originalNotFeasiblePoint, double tolerance){
		...
		RealVector x = new ArrayRealVector(originalNotFeasiblePoint.toArray()); 
		for(int i=0; i<socpConstraintParametersList.size(); i++){
 			SOCPConstraintParameters param = socpConstraintParametersList.get(i);
 			RealMatrix A = param.getA();
 			RealVector b = param.getB();
