...
//	private DoubleFactory2D F2 = DoubleFactory2D.dense;
	private static Log log = LogFactory.getLog(BasicPhaseILPPDM.class.getName());
...
//		}
		...
		protected DoubleMatrix1D rDual(DoubleMatrix1D gradF0X, DoubleMatrix1D L, DoubleMatrix1D V) {
		  ...
		  //part 2
			...
			m1 = m1.assign(gradF0X, DoubleFunctions.plus);
			
			//take into account the upper bound on s: s < maxSValue
			double m = L.getQuick((int) (L.size() - 1));
		...
		protected DoubleMatrix1D gradSum(double t, DoubleMatrix1D fiX) {
			...
			double fsU = fiX.getQuick((int) (fiX.size() - 1));
...
//				}
			...
			double LDimS = L.getQuick((int) (L.size() - 1));
			double fsU = fiX.getQuick((int) (fiX.size() - 1));
		...
		protected boolean checkCustomExitConditions(DoubleMatrix1D Xs){
			...
			boolean isInternal = (ineqX.getQuick(ineqMaxIndex) + getTolerance() <0) || Xs.getQuick((int) (Xs.size()-1))<0;
