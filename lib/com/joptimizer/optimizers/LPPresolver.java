...
//	public static final double DEFAULT_UNSPECIFIED_UPPER_BOUND = DEFAULT_UNBOUNDED_UPPER_BOUND;
	...
	/**
	 * If true, no normalization on objective function coefficients will be applied.
	 */
	private boolean avoidObjectiveFunctionNormalization = false;
	/**
...
//	private double unspecifiedUBValue = DEFAULT_UNSPECIFIED_UPPER_BOUND;
	...
	private static Log log = LogFactory.getLog(LPPresolver.class.getName());
...
//		this.unspecifiedUBValue = unspecifiedUBValue;
	...
	public void presolve(double[] originalC, 
			...
			double[] originalLB, double[] originalUB) throws JOptimizerException{
	...
	public void presolve(DoubleMatrix1D originalC, 
			...
			DoubleMatrix1D originalLB, DoubleMatrix1D originalUB) throws JOptimizerException{
		...
		this.originalN = (int) ((originalA!=null)? originalA.columns() : originalLB.size());
		...
		this.g = new double[originalMeq];
		//this.h = new double[originalN];
		this.h = new double[originalMeq];
		...
		vRowLengthMap = new int[1+originalN][];//the first position is for 0-length rows
		vColLengthMap = new int[1+originalMeq][];//the first position is for 0-length columns
	  ...
	  //check empty columns
		...
		//pre-presolving check (do not remove this)
...
//				log.debug("ub: "+ArrayUtils.toString(ub));
		...
		if(log.isDebugEnabled()){
			log.debug("final meq: " + presolvedMeq);
		}
		...
		if(!avoidObjectiveFunctionNormalization){
	...
	private void removeFixedVariables(DoubleMatrix1D c,
			...
			DoubleMatrix1D zlb, DoubleMatrix1D zub) throws JOptimizerException {
			...
			if(vRowPositionsI.length > 0){
				...
				if(forcedVariablesI.length > 0){
					...
					for(short fv =0; fv < forcedVariablesI.length; fv++){
						...
						A.setQuick(i, j, 0.);
						b.setQuick(i, 0.);
						vRowPositions[i] = removeElementFromSortedArray(vRowPositions[i], j);
						//this column has now only this nz-entry 
						changeRowsLengthPosition(i, vRowPositions[i].length+1, vRowPositions[i].length);
			...
			if(vColPositions[col].length == 1){
					...
					if(isLBUnbounded && isUBUnbounded){
						...
						for(short j = 0; j < vRowPositionsRow.length; j++){
							...
							if(column!=col && vColPositions[column].length == 1){
								//even this column is a column singleton, we do not want an empty final column
				...
				if(vRowPositionsRow.length == 2 || isImpliedFree){
					...
					if(vRowPositionsRow.length == 2){
						...
						//x = m*y + q, x column singleton (col)
						...
						if(m>0){
							...
							if(!isUBUnbounded){
								double l = ubCol/m - q/m;
								lb.setQuick(y, (isLBYUnbounded)? l : Math.max(lbY, l));
							}
							if(!isLBUnbounded){
								double u = lbCol/m - q/m;
								ub.setQuick(y, (isUBYUnbounded)? u : Math.min(ubY, u));
							}
						}
						if(vColPositions[y].length == 1){
							//this is also a column singleton, we do not want an empty final column 
							//(we are removing the row that contains the nz coefficients)
							//so we fix the value of the variable
							if(c.getQuick(y)<0){
								...
								if(isLBUnbounded(lb.getQuick(y)) && isUBUnbounded(ub.getQuick(y))){
									...
									double d = (ub.getQuick(y)+lb.getQuick(y))/2;//the mean value
									lb.setQuick(y, d);
									ub.setQuick(y, d);
	...
	private void removeDuplicateColumn(DoubleMatrix1D c,
			...
			DoubleMatrix1D ylb, DoubleMatrix1D yub,
			DoubleMatrix1D zlb, DoubleMatrix1D zub) throws JOptimizerException {
		//log.debug("actual A: " + ArrayUtils.toString(A));
	...
	private void objectiveFunctionNormalization() {
		if (this.presolvedC != null && this.presolvedC.size() > 0) {
			double normC = ALG.norm2(presolvedC);
			if (normC > 0) {
				this.presolvedC.assign(DoubleFunctions.div(normC));
			}
		}
	}
...
//			v[this.x] += this.q;
	...
	private void checkFixedVariableBounds(short x,  double v,
			DoubleMatrix1D lb, DoubleMatrix1D ub) throws JOptimizerException {
	...
	private void checkProgress(DoubleMatrix1D c,
		...
		RealMatrix AMatrix = MatrixUtils.createRealMatrix(A.toArray()); 
		RealVector Bvector = MatrixUtils.createRealVector(b.toArray());
		//log.debug("A.X-b: " + ArrayUtils.toString(originalA.operate(X).subtract(originalB)));
...
//	}
	...
	public boolean isAvoidObjectiveFunctionNormalization() {
		return avoidObjectiveFunctionNormalization;
	}

	public void setAvoidObjectiveFunctionNormalization(
			boolean avoidObjectiveFunctionNormalization) {
		this.avoidObjectiveFunctionNormalization = avoidObjectiveFunctionNormalization;
	}
