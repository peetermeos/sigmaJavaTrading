...
//	protected DoubleMatrix1D relaxedUb = null;
	...
	private static Log log = LogFactory.getLog(LPPrimalDualMethod.class.getName());
...
//	}
	 ...
	 * lb <= x <= ub
	 */
	protected void optimizeStandardLP(int nOfSlackVariables) throws JOptimizerException {
		if(log.isDebugEnabled()){
			log.debug("optimizeStandardLP");
		}
		...
		if(lpRequest.isPresolvingDisabled()){
			...
			lpPresolver.setAvoidObjectiveFunctionNormalization(lpRequest.isObjectiveFunctionNormalizationDisabled());
	 ...
	 * lb <= x <= ub
	 */
	protected void optimizePresolvedStandardLP() throws JOptimizerException {
		if(log.isDebugEnabled()){
			log.debug("optimizePresolvedStandardLP");
		}
		...
		while (true) {
		  ...
		  // progress conditions
			...
			if(getA()==null){
				...
				g = ColtUtils.sum(ColtUtils.sum(gradF0X, gradSum), ColtUtils.zMultTranspose(getA(), V, F1.make(getDim()), 0));
			}
...
//		return F2.make(ret);
	...
	protected DoubleMatrix1D getLb() {
		if (this.limitedLb == null) {
			if (super.getLb() == null) {
				this.limitedLb = F1.make((int) getC().size(), minLBValue);
			} else {
				this.limitedLb = F1.make((int) super.getLb().size());
	...
	protected DoubleMatrix1D getUb() {
		if (this.limitedUb == null) {
			if (super.getUb() == null) {
				this.limitedUb = F1.make((int) getC().size(), maxUBValue);
			} else {
				this.limitedUb = F1.make((int) super.getUb().size());
