...
//		this.unspecifiedUBValue = unspecifiedUBValue;
	...
	public void toStandardForm(DoubleMatrix1D originalC,
		...
		this.originalN = (int) originalC.size();
		if(originalLB != null && originalUB !=null){
		...
		int nOfSlackUB = 0;//number of slack variables given by the upper bounds (bounds are in fact inequalities)
		int nOfSplittedVariables = 0;//number of variables to split, x = xPlus-xMinus with xPlus and xMinus positive
		int nOfSlackLB = 0;//number of slack variables given by the lower bounds (bounds are in fact inequalities)
		...
		if(standardS==0 && nOfSplittedVariables==0){
			...
			if(originalA!=null){
				...
				standardB = F1.make((int) (standardS + originalB.size()));
			}else{
